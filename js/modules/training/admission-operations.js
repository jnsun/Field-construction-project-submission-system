...
const TrainingAdmissionOperations = {
  ...
  async openReadiness(projectId, employeeId) {
    const person = this.employee(employeeId); const { data, error } = await sb.rpc('training_admission_readiness_checklist', { p_project_id: projectId, p_employee_id: employeeId });
    if (error) { Utils.toast(error.message || '加载准入清单失败', 'error'); return; }
