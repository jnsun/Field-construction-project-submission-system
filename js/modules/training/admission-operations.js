...
const TrainingAdmissionOperations = {
  ...
  openRetrainForm(admissionId) { const a = this.state.admissions.find(x => x.id === admissionId); if (!a) return; const options = this.retrainPackageOptions(a); this.modal('发起停工复训', `<div class="alert alert-danger" style="margin-bottom:12px">该人员已被系统禁止上岗：${this.esc(a.retrain_reason || '停工超期需复训')}。</div><p class="hint">请选择由原培训包复制出来、且已替换为新项目级/专项计划并完成签发的培训包版本。旧学习、考试和签字记录不会被覆盖。</p><div class="form-group"><label>已签发复训培训包 <span class="required">*</span></label><select id="ad-retrain-package" class="form-control"><option value="">请选择新版本培训包</option>${options}</select></div><div class="form-group"><label>复训说明</label><textarea id="ad-retrain-reason" class="form-control" rows="3" placeholder="例如：项目停工后复工，课件已更新"></textarea></div>${options ? '' : '<p class="text-muted">暂无可用的新版本。请先复制原培训包，替换项目级或专项培训计划，完成审核签发。</p>'}`, `TrainingAdmissionOperations.submitRetrain('${admissionId}')`); },
  async submitRetrain(admissionId) { const pack = document.getElementById('ad-retrain-package')?.value; if (!pack) { Utils.toast('请选择已签发的新版本培训包', 'error'); return; } const { error } = await sb.rpc('training_start_pause_retraining', { p_admission_id: admissionId, p_package_id: pack, p_reason: document.getElementById('ad-retrain-reason')?.value.trim() || null }); if (error) { Utils.toast(error.message, 'error'); return; } await sb.rpc('training_send_admission_start_notice', { p_admission_id: admissionId }); this.close(); Utils.toast('已发起复训并生成待办提醒，完成补学、电子签字和现场确认前禁止上岗', 'success'); await this.load(); },

  async generateDueReminders() { if (!this.state.filter) { Utils.toast('请先选择一个项目，再按规则生成提醒', 'info'); return; } const { data, error } = await sb.rpc('training_generate_due_reminders', { p_project_id: this.state.filter }); if (error) { Utils.toast(error.message, 'error'); return; } Utils.toast(`已生成 ${data || 0} 条临期或逾期提醒`, 'success'); },
