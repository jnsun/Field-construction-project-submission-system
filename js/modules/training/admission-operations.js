...
const TrainingAdmissionOperations = {
  state: { projects: [], members: [], employees: [], packages: [], admissions: [], filter: '' },
  STATUS: {
  ...
  async load() {
    ...
    [this.state.projects, this.state.members, this.state.employees, this.state.packages, this.state.admissions] = results.map(r => r.data || []);
    this.renderTable();
  },
  ...
  row({ m, e, a }) {
    ...
    const actions = !a ? `<button class="btn btn-sm btn-primary" onclick="TrainingAdmissionOperations.openStartForm('${m.project_id}','${m.employee_id}')">发起准入</button>` : `<button class="btn btn-sm btn-secondary" onclick="TrainingAdmissionOperations.recompute('${a.id}')">刷新资格</button>${['pending_site_confirm', 'blocked'].includes(a.status) ? `<button class="btn btn-sm btn-primary" onclick="TrainingAdmissionOperations.openConfirm('${a.id}')">现场确认</button>` : ''}${a.status === 'eligible' ? `<button class="btn btn-sm btn-primary" onclick="TrainingAdmissionOperations.issue('${a.id}')">签发凭证</button>` : ''}`;
