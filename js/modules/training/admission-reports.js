...
const TrainingAdmissionReports = {
  ...
  labels: { ledger: '三级教育台账', cards: '三级安全教育记录卡', signatures: '培训签到表', exam: '考试成绩单', contractors: '外协人员台账', expiry: '凭证临期与复训名单', annual: '年度培训统计' },
  ...
  renderReport() {
    ...
    if (type === 'contractors') {
      box.innerHTML = `<div class="card"><div class="card-header"><h2>外协人员台账（${this.state.rows.length}）</h2></div><div class="card-body" style="padding:0;overflow-x:auto"><table class="data-table" style="min-width:1180px"><thead><tr><th>项目</th><th>人员 / 工种</th><th>外协单位</th><th>合同</th><th>特种作业证</th><th>在场状态</th><th>准入状态</th><th>有效至</th></tr></thead><tbody>${this.state.rows.length ? this.state.rows.map(r => `<tr><td>${Utils.escapeHtml(r.project_code)} · ${Utils.escapeHtml(r.project_name)}</td><td><b>${Utils.escapeHtml(r.employee_name)}</b><br><span class="text-muted">${Utils.escapeHtml(r.phone || '')} · ${Utils.escapeHtml(r.work_position || '未填工种')}</span><br><span class="text-muted">身份证：${Utils.escapeHtml(r.id_number || '未留存')}</span></td><td>${Utils.escapeHtml(r.contractor_name || '')}<br><span class="text-muted">${Utils.escapeHtml(r.unified_code || '')}</span></td><td>${Utils.escapeHtml(r.contract_no || '未登记')}<br><span class="text-muted">${Utils.escapeHtml(r.contract_status || '—')}</span></td><td>${Utils.escapeHtml(r.special_certificates || '无')}<br><span class="text-muted">${Utils.escapeHtml(r.certificate_status || '')}</span></td><td>${Utils.escapeHtml(r.member_status === 'active' ? '在场' : (r.member_status || '—'))}</td><td>${r.admission_status === 'not_started' ? '<span class="badge badge-danger">未发起准入</span>' : this.status(r.admission_status)}</td><td>${Utils.escapeHtml(r.valid_until || '—')}</td></tr>`).join('') : TrainingModule.emptyRow(8, '暂无外协人员记录')}</tbody></table></div></div>`;
      return;
    }
    if (type === 'expiry') {
      box.innerHTML = `<div class="card"><div class="card-header"><h2>凭证临期与年度复训名单（${this.state.rows.length}）</h2><span class="text-muted">临期天数按后台提醒设置计算</span></div><div class="card-body" style="padding:0;overflow-x:auto"><table class="data-table" style="min-width:980px"><thead><tr><th>项目</th><th>人员 / 工种</th><th>外协单位</th><th>凭证状态</th><th>当前准入</th><th>有效至 / 剩余天数</th><th>处理状态</th></tr></thead><tbody>${this.state.rows.length ? this.state.rows.map(r => `<tr><td>${Utils.escapeHtml(r.project_code)} · ${Utils.escapeHtml(r.project_name)}</td><td><b>${Utils.escapeHtml(r.employee_name)}</b><br><span class="text-muted">${Utils.escapeHtml(r.phone || '')} · ${Utils.escapeHtml(r.work_position || '未填工种')}</span></td><td>${Utils.escapeHtml(r.contractor_name || '内部员工')}</td><td>${Utils.escapeHtml(r.credential_state)}</td><td>${this.status(r.admission_status)}</td><td>${Utils.escapeHtml(r.valid_until || '—')} ${r.days_remaining == null ? '' : `<br><span class="text-muted">剩余 ${r.days_remaining} 天</span>`}</td><td>${Utils.escapeHtml(r.retraining_cycle)}</td></tr>`).join('') : TrainingModule.emptyRow(7, '暂无凭证临期或年度复训记录')}</tbody></table></div></div>`;
      return;
    }
  ...
  renderCards(box) {
    const rows = this.state.rows;
    box.innerHTML = `<div class="card"><div class="card-header"><h2>三级安全教育记录卡（${rows.length}）</h2><span class="text-muted">打印时按人员逐页生成，培训内容取自实际课件</span></div><div class="card-body" style="padding:0;overflow-x:auto"><table class="data-table" style="min-width:920px"><thead><tr><th>项目</th><th>人员</th><th>公司级</th><th>经营实体级</th><th>项目级/专项</th><th>复训档案</th><th>完整签字</th><th>现场确认</th><th>状态</th></tr></thead><tbody>${rows.length ? rows.map(r => { const levels = Array.isArray(r.levels) ? r.levels : []; const retrain = Array.isArray(r.retraining_cycles) ? r.retraining_cycles : []; const text = level => levels.filter(x => x.level === level).map(x => `${x.cycle_no > 1 ? `复训${x.cycle_no - 1}：` : ''}${x.plan_title || '未命名'}${x.completed_at ? '（已完成）' : '（未完成）'}`).join('<br>') || '—'; return `<tr><td>${Utils.escapeHtml(r.project_code)} · ${Utils.escapeHtml(r.project_name)}</td><td><b>${Utils.escapeHtml(r.employee_name)}</b><br><span class="text-muted">${Utils.escapeHtml(r.work_position || '')}</span></td><td>${text('company')}</td><td>${text('entity')}</td><td>${text('project')}${text('special') !== '—' ? `<br>${text('special')}` : ''}</td><td>${retrain.length ? retrain.map(x => `第${x.cycle_no - 1}次：${Utils.escapeHtml((x.reason || '').slice(0, 28))}`).join('<br>') : '无'}</td><td>${r.final_signed_at ? Utils.escapeHtml(String(r.final_signed_at).slice(0, 16).replace('T', ' ')) : '未签署'}</td><td>${r.site_confirmed_at ? Utils.escapeHtml(String(r.site_confirmed_at).slice(0, 16).replace('T', ' ')) : '未确认'}</td><td>${this.status(r.admission_status)}</td></tr>`; }).join('') : TrainingModule.emptyRow(9, '暂无可打印的三级教育记录卡')}</tbody></table></div></div>`;
  },
  ...
  renderAnnual(box) {
    ...
    box.innerHTML = `<div class="stats-grid">${this.card('准入记录', total, 'total')}${this.card('当前可上岗', eligible, 'success')}${this.card('综合考试通过', passed, 'info')}${this.card('年度复训', sum('annual_retraining_total'), 'warning')}${this.card('凭证临期', sum('credential_due_total'), 'warning')}${this.card('凭证失效', sum('expired_total'), 'danger')}</div><div class="card"><div class="card-header"><h2>${this.state.year}年度项目培训统计</h2></div><div class="card-body" style="padding:0;overflow-x:auto"><table class="data-table" style="min-width:1180px"><thead><tr><th>项目</th><th>准入人数</th><th>可上岗</th><th>公司级</th><th>经营实体级</th><th>项目级</th><th>专项</th><th>有效学时</th><th>考试通过/次数</th><th>复训/年度复训</th><th>临期/失效</th><th>禁止</th></tr></thead><tbody>${rows.length ? rows.map(r => `<tr><td>${Utils.escapeHtml(r.project_code)} · ${Utils.escapeHtml(r.project_name)}</td><td>${r.admission_total}</td><td>${r.eligible_total}</td><td>${r.company_completed}</td><td>${r.entity_completed}</td><td>${r.project_completed}</td><td>${r.special_completed}</td><td>${r.effective_hours}</td><td>${r.exam_passed_total} / ${r.exam_attempts_total}</td><td>${r.retraining_total || 0} / ${r.annual_retraining_total || 0}</td><td>${r.credential_due_total || 0} / ${r.expired_total || 0}</td><td>${r.blocked_total}</td></tr>`).join('') : TrainingModule.emptyRow(12, '本年度暂无项目准入记录')}</tbody></table></div></div>`;
  },
  ...
  exportCsv() {
    ...
    const columns = type === 'annual'
      ? [{ key: 'project_code', label: '项目编号' }, { key: 'project_name', label: '项目名称' }, { key: 'admission_total', label: '准入人数' }, { key: 'eligible_total', label: '可上岗人数' }, { key: 'company_completed', label: '公司级完成' }, { key: 'entity_completed', label: '经营实体级完成' }, { key: 'project_completed', label: '项目级完成' }, { key: 'special_completed', label: '专项完成' }, { key: 'effective_hours', label: '有效学时' }, { key: 'exam_passed_total', label: '考试通过' }, { key: 'exam_attempts_total', label: '考试次数' }, { key: 'retraining_total', label: '复训次数' }, { key: 'annual_retraining_total', label: '年度复训次数' }, { key: 'credential_due_total', label: '凭证临期人数' }, { key: 'expired_total', label: '凭证失效人数' }, { key: 'blocked_total', label: '禁止或失效' }]
      : type === 'cards'
      ...
      : type === 'contractors'
      ? [{ key: 'project_code', label: '项目编号' }, { key: 'project_name', label: '项目名称' }, { key: 'employee_name', label: '人员' }, { key: 'phone', label: '手机号' }, { key: 'id_number', label: '身份证号' }, { key: 'work_position', label: '工种' }, { key: 'contractor_name', label: '外协单位' }, { key: 'unified_code', label: '统一社会信用代码' }, { key: 'contract_no', label: '合同编号' }, { key: 'contract_name', label: '合同名称' }, { key: 'contract_status', label: '合同状态' }, { key: 'special_certificates', label: '特种作业证' }, { key: 'certificate_status', label: '证照审核状态' }, { key: 'member_status', label: '在场状态' }, { key: 'admission_status', label: '准入状态' }, { key: 'valid_until', label: '凭证有效至' }]
      : type === 'expiry'
      ? [{ key: 'project_code', label: '项目编号' }, { key: 'project_name', label: '项目名称' }, { key: 'employee_name', label: '人员' }, { key: 'phone', label: '手机号' }, { key: 'work_position', label: '工种' }, { key: 'contractor_name', label: '外协单位' }, { key: 'credential_state', label: '凭证状态' }, { key: 'admission_status', label: '当前准入状态' }, { key: 'valid_until', label: '凭证有效至' }, { key: 'days_remaining', label: '剩余天数' }, { key: 'retraining_cycle', label: '处理状态' }]
  ...
  printReport() {
    ...
    let head = '', body = '';
    if (type === 'annual') {
      head = '<tr><th>序号</th><th>项目</th><th>准入人数</th><th>可上岗</th><th>公司级</th><th>经营实体级</th><th>项目级</th><th>专项</th><th>有效学时</th><th>考试通过/次数</th><th>复训/年度复训</th><th>临期/失效</th><th>禁止</th></tr>';
      ...
      head = '<tr><th>序号</th><th>项目</th><th>人员/身份证号/工种</th><th>外协单位</th><th>合同</th><th>特种作业证</th><th>在场状态</th><th>准入状态</th><th>有效至</th></tr>';
      body = rows.map((r, i) => `<tr><td>${i + 1}</td><td>${esc(r.project_code)} ${esc(r.project_name)}</td><td>${esc(r.employee_name)} / ${esc(r.id_number || '未留存')} / ${esc(r.work_position || '—')}</td><td>${esc(r.contractor_name)}</td><td>${esc(r.contract_no || '未登记')}（${esc(r.contract_status || '—')}）</td><td>${esc(r.special_certificates || '无')}（${esc(r.certificate_status || '')}）</td><td>${esc(r.member_status === 'active' ? '在场' : r.member_status || '—')}</td><td>${esc(r.admission_status === 'not_started' ? '未发起准入' : this.statusText(r.admission_status))}</td><td>${esc(r.valid_until || '—')}</td></tr>`).join('');
    } else if (type === 'expiry') {
      head = '<tr><th>序号</th><th>项目</th><th>人员/工种</th><th>外协单位</th><th>凭证状态</th><th>当前准入</th><th>有效至</th><th>剩余天数</th><th>处理状态</th></tr>';
      body = rows.map((r, i) => `<tr><td>${i + 1}</td><td>${esc(r.project_code)} ${esc(r.project_name)}</td><td>${esc(r.employee_name)} / ${esc(r.work_position || '—')}</td><td>${esc(r.contractor_name || '内部员工')}</td><td>${esc(r.credential_state)}</td><td>${esc(this.statusText(r.admission_status))}</td><td>${esc(r.valid_until || '—')}</td><td>${r.days_remaining == null ? '—' : esc(r.days_remaining)}</td><td>${esc(r.retraining_cycle)}</td></tr>`).join('');
    } else {
