  ...
  async submitUser() {
    ...
    try {
      ...
      if (role !== 'reporter') {
        ...
        if (targetId) {
          const prev = isEdit
            ? !!(this.state.users.find(u => u.id === targetId) || {}).can_report
            : false;
          if (wantReport !== prev) {
            const rr = await sb.rpc('set_user_can_report', { p_user_id: targetId, p_can_report: wantReport });
            if (rr.error) {
              Utils.toast('账号已保存，但报送权限设置失败：' + this.mapRpcError(rr.error), 'error');
            }
          }
        }
      }
