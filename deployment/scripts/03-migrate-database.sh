#!/usr/bin/env bash
# =============================================================
# 03-migrate-database.sh — 云上 Supabase 数据库 → 自托管库 迁移
...
#
# 内容:
#   1. public schema 全量(结构+数据+RLS+函数) 覆盖式还原到新库
#   2. auth.users/identities/sessions/refresh_tokens 用户数据迁移(账号密码保留)
#   3. 序列值校正, 重启 auth 服务
#   4. 新库重跑 certificate-management.sql 补 Storage RLS 策略
#      （需先把 sql 文件放到 /tmp/certificate-management.sql，见脚本提示）
# =============================================================
set -euo pipefail
...
echo "== [1/4] 导出云上 public schema 并覆盖式还原 =="
...
echo "== [3/4] 业务表数量核对（新库 vs 显示成功即完成）=="
...
if [ ! -f "$CERT_SQL" ]; then
  echo "!! 未找到 $CERT_SQL"
  echo "   请先在本地执行: scp ../../sql/certificate-management.sql root@本机:/tmp/"
  echo "   或手动放入后重跑第4步: docker exec -i $DB_CONTAINER psql -U $PG_USER -d postgres < $CERT_SQL"
else
