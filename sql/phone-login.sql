...
-- --------------------------------------------------------------------------
-- ⚠️ 本节（第 4 节）的账号 RPC 已被 sql/account-rpc-v2.sql 取代
--    v2 在此基础上追加了 p_admin_level（三级管理员级别）与经营实体授权。
--    若单独重跑本文件，会把签名退回到「只有 p_phone」的旧版，
--    导致前端传 p_admin_level 时报"函数不存在"。
--    → 重跑本文件后，必须紧接着再跑一次 sql/account-rpc-v2.sql。
-- --------------------------------------------------------------------------
