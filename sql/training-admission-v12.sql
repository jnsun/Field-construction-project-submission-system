-- ============================================================================
-- 培训准入第十二批：按实际课件生成三级安全教育记录卡
-- 前置：training-admission-v1.sql 至 training-admission-v11.sql 已执行。
-- ============================================================================
...
BEGIN
  ...
  SELECT a.id, p.project_code, p.name, e.name, e.employee_no, d.name, e.position, e.phone,
         CASE WHEN public.training_is_company_admin() OR public.training_can_write(p.lead_entity_id)
              THEN e.id_number
...
-- 验证：SELECT * FROM public.training_admission_record_cards(NULL) LIMIT 1;
