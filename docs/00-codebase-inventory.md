...
| D00-R05 | 已重分类 | 历史中仅出现公开客户端 legacy `anon` API key；正式上线前迁移到 publishable/secret API keys。D05 完整角色矩阵仍按 P2 后续验证 | 不再作为服务端秘密泄露 P1；纳入上线前部署清单 | D05、发布准备 |
...
当前阶段状态：D03、D04、D05、D06 均已 PASS，安全基线门 G0 已 PASS，D07 已解锁但尚未开始。D05 登记的四个 P2 与一个 P3 继续转入对应后续专项，不因阶段签收而视为已解决。
