...
2. 生成器自动分析结构并注入控制运行时：
   - **body 顶层或某个容器（.wrap/.container 等）里存在 ≥2 个 h1/h2** → 自动按标题**切节**，
     逐节门控解锁；标题前的封面区（如 hero 大图）和页脚常驻显示，布局零破坏
   - 没有符合条件的标题 → **整页模式**：整页算一节，读到底 + 驻留满（约 360 字/分钟，
     上限 10 分钟）才能点「完成学习」；页面顶部会提示当前用的哪种模式
3. 预览确认 → 下载增强版 → 同方式上传
