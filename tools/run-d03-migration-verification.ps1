...
$ledgerResult = Invoke-D03PsqlChecked -DatabaseUrl $DatabaseUrl -Arguments @('-Atq', '-c', "SELECT migration_key FROM public.safety_schema_migrations WHERE migration_key ~ '^training-admission-v(1[7-9]|[2-4][0-9]|5[0-2])$' ORDER BY migration_key;") -OutputDirectory $runDir -Label 'ledger-count'
